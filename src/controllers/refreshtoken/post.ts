import type { RefreshTokenSchema } from "../../schema/index.js"
  ...
  app.post("/", refreshTokenMiddleware, RefreshDocs, async (c) => {
    ...
    const programs = Effect.all({
      ...
      Effect.bind("user", ({ RefreshToken, userServices }) => userServices.findOneById(RefreshToken.userId).pipe(
        Effect.catchTag("NoSuchElementException", () =>
          Effect.fail(UserErrors.FindUserByIdError.new(`Find user Error`)())),
      )),
      ...
      Effect.andThen(() => c.json({ message: "Token refreshed successfully" })),
      Effect.catchTags({
