...
export function findMany(prismaClient) {
    ...
    }).pipe(Effect.andThen(Helpers.fromObjectToSchema(ProjectSchema.SchemaArray)), Effect.withSpan("find-many.project.repository"));
}
export function findManyWithRelations(prismaClient) {
    ...
    }).pipe(Effect.andThen(Helpers.fromObjectToSchema(ProjectWithRelationsSchema.SchemaArray)), Effect.withSpan("find-many-with-relation.project.repository"));
}
export function findById(prismaClient) {
    ...
    }).pipe(Effect.andThen(Effect.fromNullable), Effect.andThen(Helpers.fromObjectToSchema(ProjectSchema.Schema)), Effect.withSpan("find-by-id.project.repository"));
}
export function findByIdWithRelation(prismaClient) {
    ...
    }).pipe(Effect.andThen(Effect.fromNullable), Effect.andThen(Helpers.fromObjectToSchema(ProjectWithRelationsSchema.Schema)), Effect.withSpan("find-many-with-relation.project.repository"));
}
