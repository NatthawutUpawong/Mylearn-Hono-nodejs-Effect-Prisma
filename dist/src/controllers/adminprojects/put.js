...
export function setupProjectPutRoutes() {
    ...
    app.put("/:ProjectId", authMiddleware, putDocssAdmin, validateRequestBody, validateUpdateParam, async (c) => {
        ...
        const programs = Effect.all({
            ...
            ? Effect.void
            : Effect.fail(UserErrors.PermissionDeniedError.new("You do not have permission to access")())), Effect.bind("existingProject", ({ ProjectService }) => ProjectService.findById(ProjectId).pipe(Effect.catchTag("NoSuchElementException", () => Effect.fail(ProjectErrors.findProjectByIdError.new(`not found Id: ${ProjectId}`)())))), Effect.bind("newName", ({ existingProject }) => Effect.succeed(body.name.trim() === ""
            ? existingProject.name
            : body.name)), Effect.tap(({ existingProject }) => body.id === existingProject.id
            ? Effect.void
            : Effect.fail(ProjectErrors.ProjectIdNotMatchError.new("Id from param and body id not match")())), Effect.andThen(({ newName, ProjectService }) => ProjectService.update(ProjectId, { ...body, name: newName })), Effect.andThen(parseResponse), Effect.andThen(data => c.json(data, 200)), Effect.catchTags({
