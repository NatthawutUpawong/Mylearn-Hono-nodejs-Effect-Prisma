...
export function setupDeleteRoutes() {
    ...
    app.delete("/:projectId", authMiddleware, deleteUserDocs, validateDeleteUserRequest, async (c) => {
        ...
        const program = ProjectServiceContext.pipe(Effect.tap(() => getUserPayload.role === "User_Admin"
            ? Effect.void
            : Effect.fail(UserErrors.PermissionDeniedError.new("You do not have permission to access")())), Effect.bind("deletedProject", ProjectServiceContext => ProjectServiceContext.findById(projectId).pipe(Effect.catchTag("NoSuchElementException", () => Effect.fail(ProjectErrors.findProjectByIdError.new(`Not found user Id: ${projectId}`)())))), Effect.andThen(svc => svc.remove(projectId)), Effect.andThen(parseResponse), Effect.andThen(data => c.json(data, 200)), Effect.catchTags({
