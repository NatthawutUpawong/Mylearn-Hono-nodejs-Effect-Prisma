...
export function setupProjectPutRoutes() {
    ...
    app.put("/:ProjectId", authMiddleware, putDocss, validateRequestBody, validateUpdateParam, async (c) => {
        ...
        }).pipe(Effect.bind("projectRelation", ({ ProjectRelationService }) => ProjectRelationService.findById(ProjectId).pipe(Effect.catchTag("NoSuchElementException", () => Effect.fail(ProRelErrors.findProjectRelationtByIdError.new(`not found ${ProjectId}`)())))), Effect.tap(({ projectRelation, UserCheckservice }) => UserCheckservice.userRoleCheckEffect(getUserPayload.role).pipe(Effect.andThen(result => result === true
            ? UserCheckservice.userIdCheckEffect(projectRelation.userId, getUserPayload.id)
            : UserCheckservice.userORGCheckEffect(projectRelation.organizationId, getUserPayload.organizationId)))), Effect.bind("existingProject", ({ ProjectService }) => ProjectService.findById(ProjectId)), Effect.bind("newName", ({ existingProject }) => Effect.succeed(body.name.trim() === ""
            ? existingProject.name
            : body.name)), Effect.tap(({ existingProject }) => body.id === existingProject.id
            ? Effect.void
            : Effect.fail(ProjectErrors.ProjectIdNotMatchError.new(`Id from param and body id not match`)())), Effect.andThen(({ newName, ProjectService }) => ProjectService.update(ProjectId, { ...body, name: newName })), Effect.andThen(parseResponse), Effect.andThen(data => c.json(data, 201)), Effect.catchTags({
