...
export function setupORGGetRoutes() {
    ...
    app.get("/", getManyDocs, async (c) => {
        ...
        const program = OrganizationServiceContext.pipe(Effect.tap(() => Effect.log("start finding many Organization")), Effect.andThen(svc => svc.findMany()), Effect.andThen(parseResponse), Effect.andThen(b => b), Effect.andThen(data => c.json(data, 200)), Effect.tap(() => Effect.log("test")), Effect.catchTags({
