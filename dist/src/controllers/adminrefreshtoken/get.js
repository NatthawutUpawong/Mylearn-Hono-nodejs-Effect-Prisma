...
export function setupRefreshtokenGetRoutes() {
    ...
    app.get("/", authMiddleware, getManyDocs, async (c) => {
        ...
        const program = RefreshTokenServiceContext.pipe(Effect.tap(() => getUserPayload.role === "User_Admin"
            ? Effect.void
            : Effect.fail(UserErrors.PermissionDeniedError.new("You do not have permission to access")())), Effect.tap(() => Effect.log("start finding many RefreshToken")), Effect.andThen(svc => svc.findManyPagination(limit, offset, page)), Effect.andThen(parseResponse), Effect.andThen(data => c.json(data, 200)), Effect.catchTags({
        ...
        }), Effect.annotateLogs({ key: "annotate" }), Effect.withLogSpan("test"), Effect.withSpan("GET /.refreshtoken.controller /"));
