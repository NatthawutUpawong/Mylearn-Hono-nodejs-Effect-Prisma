...
export function setupORGPutRoutes() {
    ...
    app.put("/:OrganizationId", authMiddleware, putDocss, validateRequestBody, validateUpdateParam, async (c) => {
        ...
        }).pipe(Effect.tap(() => userPayload.role === "User_Admin"
            ? Effect.void
            : Effect.fail(UserErrors.PermissionDeniedError.new("You do not have permission to access")())), Effect.tap(() => Effect.log("Update starting")), Effect.bind("existingORG", ({ ORGService }) => ORGService.findById(OrganizationId).pipe(Effect.catchTag("NoSuchElementException", () => Effect.fail(ORGErrors.findORGByIdError.new(`not found Id: ${OrganizationId}`)())))), Effect.bind("newName", ({ existingORG }) => Effect.succeed(body.name.trim() === "" ? existingORG.name : body.name)), Effect.tap(({ existingORG }) => body.id === existingORG.id
            ? Effect.void
            : Effect.fail(ORGErrors.ORGIdNotMatchError.new("Id from param and body id not match")())), Effect.andThen(b => b), Effect.andThen(({ newName, ORGService }) => ORGService.update(OrganizationId, { ...body, name: newName })), Effect.andThen(parseResponse), Effect.andThen(data => c.json(data, 200)), Effect.catchTags({
