...
export function setupDeleteRoutes() {
    ...
    app.delete("/:OrganizationId", authMiddleware, deleteUserDocs, validateDeleteUserRequest, async (c) => {
        ...
        const program = OrganizationServiceContext.pipe(Effect.tap(() => getUserPayload.role === "User_Admin"
            ? Effect.void
            : Effect.fail(UserErrors.PermissionDeniedError.new("You do not have permission to access")())), Effect.bind("deletedORG", OrganizationServiceContext => OrganizationServiceContext.findById(OrganizationId).pipe(Effect.catchTag("NoSuchElementException", () => Effect.fail(ORGErrors.findORGByIdError.new(`Not found user Id: ${OrganizationId}`)())))), Effect.andThen(svc => svc.remove(OrganizationId)), Effect.andThen(parseResponse), Effect.andThen(data => c.json(data, 200)), Effect.catchTags({
