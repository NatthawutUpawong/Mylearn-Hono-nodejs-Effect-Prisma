...
export function setupUserPutRoutes() {
    ...
    app.put("/:userId", authMiddleware, updateByAdminDocs, validateUpdateUserByAdminRequest, validateUpdateUserParam, async (c) => {
        ...
        const programs = Effect.all({
            ...
            ? Effect.void
            : Effect.fail(UserErrors.PermissionDeniedError.new("You do not have permission to access")())), Effect.tap(() => Effect.log("Update starting")), Effect.tap(({ ORGService }) => body.organizationId === null
            ? Effect.void
            : ORGService.findById(body.organizationId).pipe(Effect.catchTag("NoSuchElementException", () => Effect.fail(ORGErrors.findORGByIdError.new(`ORG Id: ${body.organizationId} nofound`)())))), Effect.bind("existingUser", ({ userServices }) => userServices.findOneById(userId).pipe(Effect.catchTag("NoSuchElementException", () => Effect.fail(UserErrors.FindUserByIdError.new(`Id not found: ${userId}`)())))), Effect.tap(({ existingUser }) => body.id === existingUser.id
            ? Effect.void
            : Effect.fail(UserErrors.UserIdMatchError.new("Id from param and body id not match")())), Effect.bind("updateUser", ({ projectRelationService, userServices }) => userServices.updateByAdmin(userId, body).pipe(Effect.tap(result => projectRelationService.update(result.id, ({ organizationId: result.organizationId }))), Effect.andThen(parseResponse))), Effect.tap(({ refreshtokenService, updateUser }) => refreshtokenService.findByUserId(updateUser.id).pipe(Effect.catchTag("NoSuchElementException", () => Effect.void), Effect.andThen(token => !token
            ? Effect.void
            : refreshtokenService.hardRemoveByUserId(token.userId)))), Effect.andThen(data => c.json(data.updateUser, 200)), Effect.catchTags({
