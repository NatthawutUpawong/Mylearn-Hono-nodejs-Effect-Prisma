...
export function setupUserGetRoutes() {
    ...
    app.get("/", getManyDocs, async (c) => {
        ...
        const program = UserServiceContext.pipe(Effect.tap(() => Effect.log("start finding many users")), Effect.andThen(svc => svc.findMany()), Effect.andThen(parseResponse), Effect.andThen(data => c.json(data, 200)), Effect.tap(() => Effect.log("test")), Effect.catchTags({
    ...
    app.get("/:userId", getByIdDocs, validateUserRequest, async (c) => {
        ...
        const program = UserServiceContext.pipe(Effect.tap(() => Effect.log("start finding by Id users")), Effect.andThen(svc => svc.findOneById(userId)), Effect.andThen(parseResponse), Effect.andThen(data => c.json(data, 200)), Effect.catchTags({
    ...
    app.get("all/:userId", getByIdDocs, validateUserRequest, async (c) => {
        const { userId } = c.req.valid("param");
        const parseResponse = Helpers.fromObjectToSchemaEffect(getByIdResponseSchema);
        const program = UserServiceContext.pipe(Effect.tap(() => Effect.log("start finding by Id users")), Effect.andThen(svc => svc.findallById(userId)), Effect.andThen(parseResponse), Effect.andThen(data => c.json(data, 200)), Effect.tap(() => Effect.log("test")), Effect.catchTags({
    ...
    app.get("/username/:username", getByUsernameDocs, validateusernameUserRequest, async (c) => {
        ...
        const program = UserServiceContext.pipe(Effect.tap(() => Effect.log("start finding by Username users")), Effect.andThen(svc => svc.findByUsername(username)), Effect.andThen(parseResponse), Effect.andThen(data => c.json(data, 200)), Effect.tap(() => Effect.log("test")), Effect.catchTags({
    ...
    app.get("/profile/profile", getProfileDocs, async (c) => {
        ...
        const program = Effect.succeed(token).pipe(Effect.andThen(token => token
            ...
            : Effect.fail(Errors.VerifyTokenError.new("Unauthorized")())), Effect.andThen(token => JwtServiceContext.pipe(Effect.andThen(svc => svc.VerifyToken(token)))), Effect.andThen(b => b), Effect.andThen(decoded => c.json({ decoded, message: "Profile data" })), Effect.catchTags({
