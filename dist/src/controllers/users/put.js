...
export function setupUserPutRoutes() {
    ...
    app.put("/:userId", updateUserDocs, validateUpdateUserRequest, validateUpdateUserParam, async (c) => {
        ...
        const programs = Effect.all({
            ...
            : Effect.void), Effect.catchTag("NoSuchElementException", () => Effect.void))), Effect.andThen(b => b), Effect.tap(({ existingUser, userServices }) => body.id !== existingUser.id
            ? userServices.findallById(body.id).pipe(Effect.andThen(() => Effect.fail(Errors.IdAlreadyExitError.new(`Id already exists: ${body.id}`)())), Effect.catchTag("NoSuchElementException", () => Effect.void))
            : Effect.void), Effect.andThen(({ hashedPassword, newUsername, userServices }) => userServices.update(userId, { ...body, password: hashedPassword, username: newUsername })), Effect.andThen(b => b), Effect.andThen(parseResponse), Effect.andThen(data => c.json(data, 200)), Effect.catchTags({
