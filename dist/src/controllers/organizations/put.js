...
export function setupORGPutRoutes() {
    ...
    app.put("/:ORGId", putDocss, validateRequestBody, validateUpdateParam, async (c) => {
        ...
        }).pipe(Effect.tap(() => Effect.log("Update starting")), Effect.andThen(b => b), Effect.bind("existingORG", ({ ORGService }) => ORGService.findById(ORGId).pipe(Effect.catchTag("NoSuchElementException", () => Effect.fail(Errors.findORGByIdError.new(`not found Id: ${ORGId}`)())))), Effect.andThen(b => b), Effect.bind("newName", ({ existingORG }) => Effect.succeed(body.name.trim() === "" ? existingORG.name : body.name)), Effect.andThen(b => b), Effect.andThen(({ ORGService, newName }) => ORGService.update(ORGId, { ...body, name: newName })), Effect.andThen(parseResponse), Effect.andThen(data => c.json(data, 201)), Effect.catchTags({}), Effect.withSpan("PUT /.link.controller"));
